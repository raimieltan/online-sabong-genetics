...
function obs(overrides: Partial<CoachObservation> = {}): CoachObservation {
  return {
    turn: 1,
    own: { hp: 80, maxHp: 100, stamina: 80, maxStamina: 100, momentum: 0, mentalState: "calm" },
...
test("autoCoachPolicy issues RECOVER when this fighter's own mental state is desperate", () => {
  const coach = autoCoachPolicy();
  assert.equal(coach(obs({ own: { hp: 20, maxHp: 100, stamina: 30, maxStamina: 100, momentum: -30, mentalState: "desperate" } })), "RECOVER");
});
...
test("autoCoachPolicy has no command-point gate", () => {
  const coach = autoCoachPolicy();
  assert.equal(coach(obs({ opponentContextState: "EXHAUSTED" })), "PRESS");
});
