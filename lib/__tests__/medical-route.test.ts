...
import { handleGetClinic } from "../../app/api/clinic/route";
import { handleUpgradeClinic } from "../../app/api/clinic/upgrade/route";
import { handleMedicalAction } from "../../app/api/chickens/[id]/medical/route";
...
test("GET /api/clinic returns a level-1 clinic and a roster medical overview", async () => {
  ...
  const res = await handleGetClinic(testRequirePlayer(player));
...
test("a level-1 clinic cannot treat a serious injury", async () => {
  ...
  const res = await handleMedicalAction(medicalRequest(id, { action: "treat", injuryId: "inj-serious" }), {
...
test("treating a minor injury deducts credits, then completes and clears it", async () => {
  ...
  const start = await handleMedicalAction(medicalRequest(id, { action: "treat", injuryId: "inj-minor" }), {
    params: Promise.resolve({ id }),
  }, testRequirePlayer(player));
  ...
  await handleGetClinic(testRequirePlayer(player));
...
test("POST /api/clinic/upgrade raises the clinic level when affordable", async () => {
  ...
  const res = await handleUpgradeClinic(testRequirePlayer(player));
  assert.equal(res.status, 200);
...
test("medical_rest advances recovery without a credit cost", async () => {
  ...
  const res = await handleMedicalAction(medicalRequest(id, { action: "medical_rest" }), { params: Promise.resolve({ id }) }, testRequirePlayer(player));
  assert.equal(res.status, 200);
