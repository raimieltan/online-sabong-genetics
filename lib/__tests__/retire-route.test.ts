...
test("POST /api/chickens/:id/retire sets growthStage and status to retired", async () => {
  ...
  const response = await handleRetire({ params: Promise.resolve({ id }) }, testRequirePlayer(player));
...
test("POST /api/chickens/:id/retire returns 404 for an unknown chicken", async () => {
  const player = await getOrCreateTestPlayer();
  const response = await handleRetire({ params: Promise.resolve({ id: "missing" }) }, testRequirePlayer(player));
