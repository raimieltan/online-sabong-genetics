...
type FighterDisplay = { name: string; hp: number; maxHp: number; stamina: number; tactic: string; engagement: string; intent: string; compliance?: string; awakening?: string };
...
export default function ContinuousBattle({ chickenA, chickenB, matchSeed = 81726354, autoStart = false, showControls = true, onComplete, audioEnabled = true, onToggleAudio }: { chickenA: Chicken; chickenB: Chicken; matchSeed?: number; autoStart?: boolean; showControls?: boolean; onComplete?: (result: MatchResult) => void; audioEnabled?: boolean; onToggleAudio?: () => void }) {
  ...
  return <section className={`mx-auto w-full px-3 pb-6 pt-3 sm:px-5 ${showControls ? 'max-w-[142.2vh] lg:max-w-[calc(142.2vh+232px)]' : 'max-w-[142.2vh]'}`}>
    ...
    {showControls && <div className={`mt-4 flex flex-col rounded-2xl border border-amber-300/25 bg-[linear-gradient(180deg,rgba(42,24,14,.76),rgba(12,8,5,.72))] p-3 shadow-xl backdrop-blur-xl transition-opacity duration-200 sm:p-4 lg:mt-0 lg:border-l-2 lg:border-l-amber-300/40 ${hudVisibility === 'REDUCED' ? 'opacity-85' : 'opacity-100'}`}><div className="mb-3"><p className="font-comic text-lg text-[#f5ecd8]">Utos ng Mananabong</p><p className="text-xs text-amber-100/60">Influence the next exchange. Your rooster still decides.</p><p className={`mt-2 rounded-full border px-3 py-1 text-center text-[10px] font-semibold uppercase tracking-[.12em] ${decisionWindow && display.cooldown <= 0 ? 'border-emerald-300/30 bg-emerald-950/30 text-emerald-200' : 'border-amber-300/20 bg-black/30 text-amber-200'}`}>{commandStatus}</p>{left?.compliance && <p className="mt-2 text-center text-[10px] uppercase tracking-[.16em] text-amber-100/70">{left.compliance} · {left.intent.replaceAll('_', ' ')}</p>}{left?.awakening && <p className="mt-2 text-center font-display text-sm uppercase tracking-[.16em] text-[#f1cf77]">Awakened · {left.awakening.replaceAll('-', ' ')}</p>}</div><div className="grid grid-cols-3 gap-2 sm:gap-3 lg:grid-cols-1" aria-label="Tactical commands">{TACTICAL_MODES.map(mode => { const card = COMMAND_CARDS[mode] ?? COMMAND_CARDS.balanced, active = display.fighters[0]?.tactic === mode; return <button key={mode} aria-pressed={active} disabled={display.phase !== 'active' || display.cooldown > 0} onClick={() => command(mode)} className={`relative min-h-24 rounded-xl border p-2 text-left transition active:scale-[.98] disabled:cursor-not-allowed disabled:opacity-45 sm:min-h-28 sm:p-3 ${card.tone} ${active ? 'ring-2 ring-amber-300 ring-offset-2 ring-offset-[#1a100b]' : ''}`}><span className="block text-2xl sm:text-3xl">{card.icon}</span><span className="mt-1 block font-comic text-sm text-white sm:text-base">{card.title}</span><span className="block text-[10px] text-white/60 sm:text-xs">{card.subtitle}</span></button>; })}</div></div>}
    </div>
