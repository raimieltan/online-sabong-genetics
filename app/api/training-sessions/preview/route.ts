...
export async function POST(request:Request){
  ...
  try{const player=await requirePlayer();return NextResponse.json(await previewTrainingForChicken(player.id,body.chickenId,body.programId as ProgramId,category,intensity));}catch(error){if(error instanceof FacilityError)return NextResponse.json({error:error.code},{status:error.status});return toErrorResponse(error);}
}
