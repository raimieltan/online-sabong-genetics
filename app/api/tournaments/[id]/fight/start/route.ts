...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const combatView = await createSession({ fighterId: chicken.id, encounterId: encounter.id, coachingMode: "MANUAL", openingCommand: "WAIT", disconnectPolicy: "KEEP_INSTRUCTION", idempotencyKey: encounter.id }, player.id);
