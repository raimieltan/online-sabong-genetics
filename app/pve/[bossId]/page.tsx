...
export default function BossEncounterPage({ params }: { params: Promise<{ bossId: string }> }) {
  ...
  useEffect(() => { Promise.all([fetch("/api/pve/bosses").then((r) => r.json()), fetch("/api/chickens").then((r) => r.json())]).then(([pve, roster]) => { setEntry(pve.bosses.find((item: BossListEntry) => item.boss.id === bossId) ?? null); setChickens(roster); }); }, [bossId]);
