...
type Sort = "all" | "identity" | "record" | "traits";
...
export function ParentSelector({ role, options, selectedId, onSelect, onClose }: {
  ...
  const filtered = useMemo(() => {
    const matches = options.filter((chicken) => `${chicken.name} ${chicken.breed ?? ""} ${presentFighterIdentity(chicken).primaryLabel}`.toLowerCase().includes(query.toLowerCase()));
    if (sort === "identity") return matches.sort((a, b) => presentFighterIdentity(a).primaryLabel.localeCompare(presentFighterIdentity(b).primaryLabel));
    if (sort === "record") return matches.sort((a, b) => b.record.championships - a.record.championships || b.record.wins - a.record.wins);
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label={`Select ${title}`} onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="breeding-selector w-full max-w-5xl rounded-t-2xl sm:rounded-2xl">
        ...
        <div className="border-b border-white/5 px-5 py-3">
          ...
          <div className="mt-2 flex gap-1.5 overflow-x-auto pb-1">{([['all', 'All stock'], ['identity', 'Fighter identity'], ['record', 'Proven lines'], ['traits', 'Trait lines']] as [Sort, string][]).map(([value, label]) => <button key={value} type="button" onClick={() => setSort(value)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider ${sort === value ? "border-(--color-gold-bright) bg-(--color-gold)/15 text-(--color-gold-bright)" : "border-white/10 bg-black/20 text-(--color-text-muted)"}`}>{label}</button>)}</div>
        </div>
        <div className="grid max-h-[62vh] grid-cols-2 gap-2 overflow-y-auto p-4 sm:grid-cols-3 lg:grid-cols-4">
          {filtered.map((chicken) => {
            ...
            return <button key={chicken.id} type="button" onClick={() => { onSelect(chicken.id); onClose(); }} className={`group rounded-xl border p-3 text-left transition ${selected ? "border-(--color-gold-bright) bg-(--color-gold)/15 shadow-[0_0_24px_rgba(218,163,64,.12)]" : "border-white/8 bg-black/25 hover:border-(--color-gold)/45 hover:bg-black/35"}`}><ChickenThumbnail chicken={chicken} className="h-28 w-full rounded-lg bg-[radial-gradient(ellipse_at_center,rgba(212,162,78,.14),transparent_70%)] sm:h-36" /><div className="mt-2 flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate font-display text-sm text-(--color-parchment)">{chicken.name}</p><p className="mt-0.5 truncate text-[8px] uppercase tracking-wide text-(--color-text-muted)">{identity.primaryLabel} · Gen {chicken.generation}</p></div><span className="text-[8px] uppercase text-(--color-gold-bright)">{topRarity(chicken.traits)}</span></div><p className="mt-2 text-[9px] leading-relaxed text-(--color-text-muted)">{identity.strengths[0]}</p><p className="mt-1 text-[8px] uppercase tracking-wide text-(--color-gold-bright)">{chicken.record.wins}W–{chicken.record.losses}L · {chicken.record.championships} titles</p></button>;
          })}
